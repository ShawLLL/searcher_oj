...
public:
    static void ParseBody(const std::string& body,std::unordered_map<std::string,std::string>*params){
        ...
        for(size_t i = 0;i<kvs.size();++i)
        {
            ...
            //unordered_map[]操作的行为：如果可以不存在，就新增
...
/*
   class StringUtil{
   public:
// aaa bbb ccc 按照一个空格切分应该是三个部分
// aaa  bbb ccc中间是两个空格，如果不对空格进行压缩，则回识别是四个部分
// boost::token_compress_off 是对空格压缩的关闭，则回切分成四个部分
// boost::token_compress_on 是对空格压缩的打开，则会切分成三个部分
static void Split(const std::string& input,const std::string& split_char,std::vector<std::string>* output){
